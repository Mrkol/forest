...
s32 __CARDSetStatusExAsync(s32 chan, s32 fileNo, struct CARDDir * dirent, void (* callback)(s32, s32)) {
    ...
    ASSERTLINE(139, *dirent->fileName != 0xff && *dirent->fileName != 0x00);
    ASSERTMSGLINE(147, dirent->iconAddr == 0xffffffff || dirent->iconAddr < CARD_READ_SIZE, "CARDSetStatus[Async](): stat->iconAddr must be 0xffffffff or less than CARD_READ_SIZE.");
    ASSERTMSGLINE(150, dirent->commentAddr == 0xffffffff || (dirent->commentAddr & 0x1FFF) <= 8128, "CARDSetStatus[Async](): comment strings (set by stat->commentAddr) must not cross 8KB byte boundary.");
