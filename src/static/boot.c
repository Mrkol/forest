 ...
 * @return void* NULL
 */
void* soundArenaAlloc(size_t size) {
    return NULL;
}
    ...
    if (result) {
        ...
        if (bss != NULL) {
            OSReport("サウンドアリーナ %08x 使用 bss=%08x\n", module->bssSize,
                     bss); /* Sound Arena %08x used bss=%08x */
...
HMODULE LoadLink(const char* module_name) {
    ...
    void (*foresta_main)() = (void (*)())GetProcAddress(module, "main");
    if (foresta_main != nullptr) {
        foresta_main();
    }

    u32 (*foresta_entry)() = (u32 (*)())GetProcAddress(module, "entry");
    if (foresta_entry != nullptr) {
        HotStartEntry = (void*)foresta_entry;
    }
...
void fault_callback_vimode() {
    ...
    if (!APPNMI_TESTMODE_GET()) {
        OSReport("貸し出しバージョンなのでコードは出しません\n"); /* Since this is a loaned version, we will not provide
                                                                     the code */
...
#define OVERLINE(con) \
    (((JC_JUTConsole_getUsedLine(con) - JC_JUTConsole_getHeight(con)) + 1) <= JC_JUTConsole_getLineOffset(con))
    ...
    if (!pad_disabled) {
        ...
        do {
            ...
            if (button == PAD_BUTTON_UP) {
                JC_JUTConsole_scroll(console, (up_speed < 3) ? -1 : ((up_speed < 5) ? -2 : ((up_speed < 7) ? -4 : -8)));
                update_console = TRUE;
                down_speed = 0;
                up_speed++;
            } else if (button == PAD_BUTTON_DOWN) {
                JC_JUTConsole_scroll(console,
                                     (down_speed < 3) ? 1 : ((down_speed < 5) ? 2 : ((down_speed < 7) ? 4 : 8)));
                update_console = TRUE;
                up_speed = 0;
                down_speed++;
            } else {
...
int main(int argc, const char** argv) {
    ...
    static const boot_tbl_t tbl = { "L+R+X+Y+Down, START BUTTON",
    ...
    if (APPNMI_RESETEXEMPT2_GET()) {
        ...
        OSReport("異常状態でのリセット検出ゆえリセットさん免除フラグをセットしました" VT_RST
    ...
    fault_AddClientEx(&my_fault_client1, (FaultCallback)fault_callback_keyCheck, tbl.msg,
                      (tbl.param_upper << 16) | tbl.param_lower, 10, 14);
